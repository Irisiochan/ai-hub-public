...
用环境变量（`HUB_PORT/HUB_HOST/HUB_TOKEN/HUB_DATA_DIR/HUB_WEB_DIST/HUB_CONFIG`）
...
- **远程模式**：`%APPDATA%/ai-hub/desktop.json` 写
  `{"remoteUrl": "http://100.x.y.z:3900"}`（填自己的 Tailscale 地址），壳不起本地网关，直接套现有
...
- worker / hub-mcp 端点维持自己的 Bearer 鉴权。
