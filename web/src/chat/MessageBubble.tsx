...
import type { Contact, Message, UserProfile } from '../platform/api';
import { shouldOpenInExternalView } from './externalLinks';
import { withBase } from '../platform/mobileShell';
import { isManualUserMessage } from './messageSource.ts';
import { outputLimitWarning } from './messageWarnings';
import { formatMessageTimestamp } from '../platform/time';
import { displayedErrorContent } from './interruptionReason';
import { parseHandoffObligationMeta } from '../roomTasks/handoffObligation';
import HandoffObligationAlert from '../roomTasks/HandoffObligationAlert';
...
function MessageBubble({
  ...
  if (message.kind === 'error') {
    // Handoff-obligation boxes link to the exact failed turn + tasks: a later
    // valid disposition on the same chain flips the box to recovered with a
    // real details entry, instead of staying red forever.
    if (parseHandoffObligationMeta(message.meta)) {
      return (
        <div className={`${groupClass} center`}>
          <HandoffObligationAlert
            message={message}
            fallback={displayedErrorContent(message)}
            bulkClass={bulkClass}
            bulkMark={bulkMark}
            selected={selected}
