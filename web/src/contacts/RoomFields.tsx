import { type Contact } from '../platform/api';
  ...
  return (
    <>
      <div className="cfg-group">
        ...
        <p className="cfg-note">{props.workflowEnabled
          ? '这是工作会议室。成员可作为预备 agent；只有当前模块绑定者接收相应任务，未绑定者不唤醒。模型与强度在会议室的「工作流模块」面板选择。'
          : '群里用 @名字 点名，@all 叫全员；默认无 @ 时不调用模型，避免无意消耗。'}</p>
      </div>

      {!props.workflowEnabled && <div className="cfg-group">
        <h3>发言规则</h3>
