...
import { api, type Contact, type ModelOption } from '../platform/api';
import ApiFields from './ApiFields';
import CliFields from './CliFields';
...
export default function ContactConfig({ contact, contacts, onClose }: Props) {
  ...
  const [delegRunners, setDelegRunners] = useState<string[]>(
    Array.isArray(delegation.runners) && delegation.runners.length
      ? delegation.runners.filter((runner: unknown) => ['claude', 'codex', 'grok', 'opencode'].includes(String(runner)))
          ...
          <div className="cfg-body">
            {activeTab === 'room' && (
              <>
                {identity}
                <RoomFields
                  workflowEnabled={contact ? isWorkflowRoom(contact) : false}
                  contacts={dmContacts}
                  members={members}
