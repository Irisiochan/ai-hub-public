import type { JobMessage, Worker, WorkerJob, WorkflowProfilesResponse, WorkflowModulesResponse } from '../platform/api';
...
type Client = {
  ...
  workflowModules?(): Promise<WorkflowModulesResponse>;
};
...
export interface WorkerState {
  ...
  modules: WorkflowModulesResponse | null;
  messages: Record<string, JobMessage[]>;
  errors: Record<string, string>;
  /** In-flight REST reads by the same keys used in errors (jobs/workers/profiles/modules/job:<id>). */
  loading: Record<string, boolean>;
  /** True after the first successful snapshot for that key; distinguishes empty from not-yet-loaded. */
  loaded: Record<string, boolean>;
}

const emptyState = (): WorkerState => ({ workers: [], jobs: [], workflow: null, modules: null, messages: {}, errors: {}, loading: {}, loaded: {} });
...
export function createWorkerState(client: Client) {
  ...
  const load = <T>(key: string, read: () => Promise<T>, apply: (data: T, at: number) => void,
    ...
    if (!state.loading[key]) publish({ loading: { ...state.loading, [key]: true } });
    const request = Promise.resolve().then(read).then((data) => {
      if (!current()) return;
      apply(data, at);
      const patch: Partial<WorkerState> = {};
      if (state.errors[key]) patch.errors = { ...state.errors, [key]: '' };
      if (!state.loaded[key]) patch.loaded = { ...state.loaded, [key]: true };
      if (Object.keys(patch).length) publish(patch);
    }).catch((error: unknown) => {
      if (current()) publish({ errors: { ...state.errors, [key]: (error as Error).message } });
      throw error;
    }).finally(() => {
      // 只有仍是当前在途请求的结束才能清掉该 key 的 loading：旧请求被取代、
      // 或 pending 已被卸载删掉时，不得把新请求的 loading 置为 false。
      if (pending.get(key) !== request) return;
      pending.delete(key);
      if (epoch === generation && state.loading[key]) publish({ loading: { ...state.loading, [key]: false } });
    });
  ...
  const applyModules = (modules: WorkflowModulesResponse, at = ++revision) => {
    if (at < modulesSnapshotAt || modules.revision < (state.modules?.revision ?? 0)) return;
    modulesSnapshotAt = at;
    publish({ modules });
  };
  const refreshModules = () => client.workflowModules
    ? load('modules', () => client.workflowModules!(), applyModules)
    : Promise.resolve();
  ...
  const refresh = () => Promise.all([
    refreshWorkers(), refreshJobs(), refreshProfiles(), refreshModules(), ...[...watching.keys()].map(refreshJob),
  ]).then(() => {});
  ...
  return {
    ...
    watchJob(id: string) {
      ...
      return () => {
        ...
        // 卸载清掉当前这个 viewer 自己的 detail loading，避免无新请求时 loading 常亮；
        // 若卸载后又起了新请求，新请求会重新置位，旧请求的 finally 不得再清掉它。
        const loading = { ...state.loading };
        if (loading[`job:${id}`]) delete loading[`job:${id}`];
        publish({ messages, errors, loading });
      };
    },
    reset() {
      generation += 1;
