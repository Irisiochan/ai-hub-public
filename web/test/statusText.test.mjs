...
assert.equal(
  statusText({ state: 'thinking', member: '嘎啦蜜' }, { isRoom: true, contactName: '会议室' }),
  '嘎啦蜜 思考中…'
);
...
assert.equal(
  statusText({ state: 'streaming', member: '嘎啦蜜' }, { isRoom: true, contactName: '会议室' }),
  '嘎啦蜜 正在输入…'
);
assert.equal(
  statusText({ state: 'tool:search_vault', member: '嘎啦蜜' }, { isRoom: true, contactName: '会议室' }),
  '嘎啦蜜 正在用 search_vault'
);
