...
export { parseHubTimestampMs } from '../lib/hub-time.mjs';
...
/** Queued daily / idea / followup events drain as noop. Diary and reminders stay. */
export function isRemovedCompanionEvent(event) {
  if (!event || typeof event !== 'object') return false;
  if (isTaskReminderMode(event) || isDiaryMode(event)) return false;
  if (isDailyMode(event) || isIdeaMode(event)) return true;
  if (event.source === 'followup-sweep' || event.source === 'followup') return true;
  const payload = event.payload;
  if (payload && typeof payload === 'object' && !Array.isArray(payload)) {
    if (payload.mode === 'followup' || payload.followupId) return true;
  }
  return false;
}

export function normalizeTaskReminderConfig(raw = {}) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    throw new Error('taskReminders must be an object');
  }
  const recipient = String(raw.recipient ?? 'claude').trim().toLowerCase();
  if (!recipient) {
    throw new Error('taskReminders.recipient must be a contact id');
  }
  return {
...
/**
 * Workflow-only background operation (2026-09-12, user: 每日额度删掉，除开维持
 * 会议室和worker正常运行的功能，其余主动发消息和要ds跑的功能全关).
 *
 * Persisted master boundary: `workflowOnly.enabled === true` means only formal
 * workflow execution may send messages or call DS:
 * - KEEP: coordination execution (Plan-ready) + due verification dispatches,
 *   required outcome/vault-outbox/job lifecycle, health/metrics, binding
 *   snapshots (task reread, fingerprint, idempotency, TTL).
 * - OFF: diary rollup, task reminders/nudges,
 *   backlog sweep, agenda shadow, route triage (+autoDispatch),
 *   hub-auto hygiene, and all generic backlog/system-timer L1 triage + nudges.
 *   Idea rooms, absence followups, and proactive daily check-ins are removed.
 *
 * Semantics:
 * - Config-only disabling producers is NOT sufficient. All consumption paths
 *   must also guard: pre-existing queued/retry events, manual flags
 *   (--sweep/--agenda/--route-triage/--task-reminders), and webhook POST
 *   /event with forged category/mode must never reach DS or Hub dispatch
 *   when workflowOnly is on. They finish as `noop` (ledger preserved, never
 *   erased).
 * - Coordination `dailyLimit` no longer gates execution/verification (see
 *   WORKFLOW_EXECUTION_SCAN_BATCH_LIMIT): formal dispatches are bounded per
 *   scan + retry-safe, never daily-capped. Ancillary paths that still read
 *   coordinationPolicyState keep their quota in non-workflowOnly mode, but
 *   are fully suppressed when workflowOnly is on.
 * - Explicit user chat to a DS model is out of scope; this only gates
 *   background automation.
 */
export function normalizeWorkflowOnlyConfig(raw = {}) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    throw new Error('workflowOnly must be an object');
  }
  return {
    enabled: raw.enabled === true,
  };
...
export function isWorkflowOnlyConfig(config = {}) {
  return config?.workflowOnly?.enabled === true;
}

/**
 * Retired: the formal host sweep no longer enqueues execution/verification,
 * so no event is workflow-essential anymore. Kept as a named predicate
 * (always false) so call sites and tests keep compiling while the pipeline
 * drains legacy payloads explicitly. Hygiene stays non-essential as before.
 */
export function isWorkflowEssentialEvent(event) {
  void event;
  return false;
}

/**
 * Execution/verification no longer consume a numeric daily quota. Each scan is
 * still bounded so a tasksDir with hundreds of files cannot enqueue
 * unbounded work in one tick; retries remain bounded by maxAttempts/claim
 * timeout. This is scan batching, not a daily cap.
 */
export const WORKFLOW_EXECUTION_SCAN_BATCH_LIMIT = 20;

/**
 * Formal execution gate: enabled + roomId + tasksDir. Intentionally ignores
 * coordination.dailyLimit (deprecated for execution; kept parsed for compat).
 */
export function coordinationExecutionAllowed(coordination) {
  const config = normalizeCoordinationConfig(coordination ?? {});
  return Boolean(config.enabled && config.roomId && config.tasksDir);
}
...
const EXPLICIT_REJECTION_PATTERNS = [
  ...
  // (bare 发了 removed: "别 + 0–6 字 + 发了" glued engineering text like "先别收，开发了吧";
  // 发-message collocations stay explicit: 发这个/发这种/发这些/发消息)
  /(?:别|不要|不用|停止|取消|关掉)[^。！？；\n\r]{0,6}(?:推送|派单|提醒|打扰|叫我|主动(?:发|推送|提醒|消息|派)|发这个|发这种|发这些|发消息)/u,
...
// Suppressor: engineering instructions must never read as stop-delivery.
// Vetoes rejected when the hit text mentions 开发 / 先别收 / 别收
// (M1 false positive: "@Claude 先别收，开发了吧" reply#4203, room cmrhxny03).
const OUTCOME_REJECTION_SUPPRESSOR = /(?:开发|先别收|别收)/u;
...
export function classifyOutcomeMessage(message) {
  ...
  if (OUTCOME_REJECTION_SUPPRESSOR.test(content)) return OUTCOME_LABEL_ENGAGED;
...
/**
 * Plan 正文兜底的执行人声明：vault 的 add_task/update_task 写不进 frontmatter
 * executor，room Plan 只能把 `executor: <联系人>` 写在 `### 执行者与工作区`
 * 区段内（或无该区段时整个 Plan 内）。frontmatter 仍优先；两者并存必须一致，
 * 多个不同声明一律 fail closed；下游网关 gate 的 executor 比对、workspace、
 * 指纹、TTL、房间隔离与幂等保持不变。
 */
function planBodyExecutors(scope) {
  return [...String(scope ?? '').matchAll(/^\s*(?:[-*]\s+)?executor\s*:\s*["']?([A-Za-z0-9][A-Za-z0-9_-]{0,79})["']?\s*$/gim)]
...
export function parseCoordinationTask(raw, { taskPath = '' } = {}) {
  ...
  const frontmatterExecutor = String(frontmatter.executor ?? '').trim().toLowerCase();
  ...
  const bodySet = new Set(planBodyExecutors(workspaceScope));
  if (bodySet.size > 1) return null;
  const bodyExecutor = bodySet.size === 1 ? [...bodySet][0] : '';
  if (frontmatterExecutor && bodyExecutor && frontmatterExecutor !== bodyExecutor) return null;
  const executor = frontmatterExecutor || bodyExecutor;
...
export function formatCoordinationDispatchBlock(task) {
  ...
  return [
    ...
    'runner/model/reasoning 不由本消息决定：服务端按当前 execute 模块绑定捕获，网关 pin 到可信 dispatch；不要按自己后端自选 runner，只在本轮可信 dispatch 内用工具，workspace 使用上面的固定值。',
    'delegate_to_worker.prompt 必须逐字使用下面模板，不允许改写成自由文本任务描述：',
    '---BEGIN FIXED PROMPT---',
    prompt,
    '---END FIXED PROMPT---',
    `执行回执回来后按回执阶段处理并写清 blocker；不要自行替 User 关闭任务，不要在群里用 PASS 给验收结论。正式结论以 review 模块的 candidate-bound 审查为准，preview 只是索引不给 verdict。`,
  ].join('\n');
}
...
export function chooseRecipient({
  ...
  const allowed = Array.isArray(allowedRecipientKeys) && allowedRecipientKeys.length
    ...
    : null;
  // modelOnly: never let the static rules table override an already chosen recipient.
