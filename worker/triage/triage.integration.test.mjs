...
test('retired coordination sweep enqueues and dispatches nothing', async () => {
  ...
  try {
    ...
    assert.equal(deepseekCalls, 0, 'retired scan must not call L1');
    assert.equal(dispatched.length, 0, 'retired scan must not dispatch even with eligible Plan files');
    // Task-file parsers stay intact for explicit import paths.
    const firstPlan = parseCoordinationTask(
      fs.readFileSync(path.join(tasksDir, 'eligible.md'), 'utf8'),
      { taskPath: 'tasks/eligible.md' },
    );
    assert.equal(firstPlan.taskPath, 'tasks/eligible.md');
    assert.equal(firstPlan.executor, 'codex');

    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(dispatched.length, 0, 'reruns stay quiet too');

    fs.writeFileSync(path.join(tasksDir, 'eligible.md'), taskBody('npm test && npm run build'));
    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(dispatched.length, 0, 'revised Plans never re-arm the retired sweep');

    const store = new TriageStore(stateFile);
    try {
      assert.equal(store.dailySummary().coordinationPoolDispatched, 0);
      assert.equal(store.dailySummary().coordinationExecutionDispatched, 0);
      assert.equal(store.dailySummary().coordinationVerificationDispatched, 0);
      assert.equal(store.dailySummary().ideaPoolDispatched, 0);
      assert.equal(
        store.db.prepare("SELECT COUNT(*) AS c FROM triage_events WHERE source = 'coordination-sweep'").get().c,
        0,
        'no sweep events are ever enqueued',
      );
    } finally {
      store.close();
    }
...
test('retired verification scan dispatches nothing for due open tasks', async () => {
  ...
  try {
    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(deepseekCalls, 0, 'retired scan must not call L1');
    assert.equal(dispatched.length, 0, 'retired verification scan dispatches nothing even for due open tasks');
    ...
    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(dispatched.length, 0, 'changed due dates never re-arm the retired sweep');
    ...
    try {
      ...
      assert.equal(summary.coordinationVerificationDispatched, 0);
      assert.equal(
        store.db.prepare("SELECT COUNT(*) AS c FROM triage_events WHERE source = 'coordination-sweep'").get().c,
        0,
        'no sweep events are ever enqueued',
...
test('queued legacy coordination payloads drain as noop without dispatch', async () => {
  ...
  try {
    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    // All three queued legacy payloads (stale execution, closed execution,
    // stale verification) drain as noop: no Hub sends, no model wake.
    assert.equal(dispatched.length, 0, 'retired consumer must not dispatch for any queued legacy payload');

    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(dispatched.length, 0, 'settled noop state must stay quiet on rerun');

    const store = new TriageStore(stateFile);
    try {
      const terminal = store.db.prepare(
        "SELECT COUNT(*) AS c FROM triage_events WHERE status IN ('finished', 'dispatched', 'noop')",
      ).get().c;
      assert.equal(terminal, 3, 'all legacy payloads reach a terminal state without dispatch');
      assert.equal(store.dailySummary().coordinationPoolDispatched, 0);
      // Legacy v1 state rows are preserved untouched (ledger preserved, never
      // erased or migrated into new dispatches).
      const state = JSON.parse(store.getSourceState('coordination:v1'));
      assert.equal(state['tasks/legacy.md'], legacyPlan.planHash);
      assert.equal(
        store.getSourceState(`verification:v2:tasks/verif-stale.md:${yesterday}:codex`),
        null,
        'no verification state is created by the retired consumer',
      );
    } finally {
      store.close();
    }
...
test('crashed legacy coordination payload replays into noop without visible dispatch', async () => {
  ...
  try {
    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(posts.length, 0, 'retired scan enqueues nothing on the first pass');

    // 崩溃注入：模拟崩溃前遗留的已排队 coordination 事件（本地未收口）。
    const crashed = new TriageStore(stateFile);
    let eventId = '';
    try {
      const seeded = crashed.enqueue({
        source: 'coordination-sweep',
        categoryHint: 'coordination',
        summary: 'legacy coordination payload stranded by a crash',
        dedupeKey: 'legacy-crash-payload',
        payload: { mode: 'coordination', task: parseCoordinationTask(
      ...
      eventId = seeded.id;
      crashed.db.prepare(
        "UPDATE triage_events SET status = 'queued', next_attempt_at = 0 WHERE id = ?",
      ).run(eventId);
    } finally {
      crashed.close();
    }
    ...
    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(posts.length, 0, 'replay must drain as noop without any visible dispatch');

    await runWorker(configPath, { TEST_DEEPSEEK_KEY: 'test-only' });
    assert.equal(posts.length, 0, 'settled noop state must stay quiet afterwards');
    ...
    try {
      assert.equal(
        ...
        'noop',
        'crashed legacy payload reaches a terminal noop state',
      );
      assert.equal(store.poolUsage(DELIVERY_POOL_COORDINATION).count, 0, 'replay burns no pool');
      assert.equal(
        store.db.prepare('SELECT COUNT(*) AS c FROM triage_deliveries WHERE event_id = ?').get(eventId).c,
        0,
        'no delivery row is recorded for a retired drain',
      );
      assert.equal(store.dailySummary().coordinationExecutionDispatched, 0);
    } finally {
      store.close();
