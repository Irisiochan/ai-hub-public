...
test('retired auto-dispatch holds suggestions without dispatching or claiming', async () => {
  const { worker, store, hostDispatches, cleanup } = await makeWorker({
    autoDispatch: AUTO,
  ...
  try {
    ...
    assert.equal(acted, false, 'no veto and no dispatch: nothing acted upon');
    assert.equal(hostDispatches.length, 0, 'retired auto-dispatch never sends');
    const row = store.db.prepare(
      "SELECT status, resolved_recipient, resolved_via FROM route_suggestions WHERE item_path = 'tasks/2026-08-20_alpha.md'",
    ).get();
    assert.equal(row.status, 'pending', 'suggestion stays pending for manual handling or expiry');
    assert.equal(row.resolved_recipient, null);
    assert.equal(row.resolved_via, null);
    assert.equal(worker.claims['tasks/2026-08-20_alpha.md'], undefined, 'no backlog claim is taken');
    // Re-running stays quiet too.
...
test('route-auto round with spoke=0 releases its claim, marks passed and leaves task open', async () => {
  ...
  try {
    // Legacy dispatched rows (from before auto-dispatch retired) still
    // reconcile the same way: no new dispatches are created to get here.
    ...
    store.markRouteSuggestionDispatched(seeded.id, {
      resolvedRecipient: 'codex',
      dispatchMessageId: 501,
...
test('retired auto-dispatch leaves all suggestion rows pending regardless of stage', async () => {
  const { worker, store, tasksDir, hostDispatches, cleanup } = await makeWorker({
  ...
  try {
    ...
    // Retired: no suggestion dispatches regardless of stage, title safety,
    // mode, age, or daily limit. Everything stays pending for manual
    // handling or expiry; the veto/dispatch bookkeeping above is untouched.
    assert.equal(hostDispatches.length, 0);
    ...
    assert.equal(statuses['tasks/2026-08-20_alpha.md'], 'pending');
