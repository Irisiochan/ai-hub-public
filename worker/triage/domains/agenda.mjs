...
import { isWorkflowOnlyConfig, nextWallClockDelay, shanghaiDateAt } from '../triage-core.mjs';
import { agendaOnce, log, once } from '../domain-shared.mjs';
...
export const agendaMethods = {
  ...
  async runAgendaShadowOnce(now = Date.now()) {
    // Workflow-only: Agenda digest is ancillary auto work (posts to room).
    if (isWorkflowOnlyConfig(this.config)) {
      log('info', 'agenda shadow quiet', { reason: 'workflow-only' });
  ...
  startAgenda() {
    if (isWorkflowOnlyConfig(this.config)) {
      log('info', 'workflow-only: agenda timer disabled');
      return null;
    }
    const config = this.agendaConfig();
