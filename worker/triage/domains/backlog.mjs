...
import { BACKLOG_CLAIMS_KEY, log, once, sweepOnce } from '../domain-shared.mjs';
...
export const backlogMethods = {
   ...
   * 走普通 backlog 事件，沿用现成的池子与去重，不另开一条投递通道。
   */
  async sweepBacklogInbox() {
    // Workflow-only:待拆分需求日扫 is ancillary proactive work. Forced off.
    if (isWorkflowOnlyConfig(this.config)) {
      log('info', 'workflow-only: backlog sweep suppressed');
      return null;
    }
  ...
  startBacklogSweep() {
    if (isWorkflowOnlyConfig(this.config)) {
      log('info', 'workflow-only: backlog sweep timer disabled');
      return null;
    }
    const config = this.backlogSweepConfig();
    if (!config.enabled) return null;
