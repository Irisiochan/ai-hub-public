...
import { log, once, reminderOnce, reminderShadow } from '../domain-shared.mjs';

/** TriageWorker 的任务到期提醒 domain：确定性截止日期扫描与会议室/主窗路由。 */
export const reminderMethods = {
  taskReminderConfig() {
    return this.config.taskReminders ?? normalizeTaskReminderConfig({});
  },
  ...
  async scanTaskReminders({ shadow = false } = {}) {
    // Workflow-only: deterministic due/overdue blasts are ancillary. Only the
    // read-only --reminder-shadow probe (no enqueue, no DS, no send) stays.
    if (isWorkflowOnlyConfig(this.config) && !shadow) {
      log('info', 'workflow-only: task reminder scan suppressed');
      return { reason: 'workflow-only', reminders: [] };
    }
  ...
  async startTaskReminders() {
    ...
    if (isWorkflowOnlyConfig(this.config)) {
      log('info', 'workflow-only: task reminder timer disabled');
      return;
    }
