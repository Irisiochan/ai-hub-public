...
import { closureBearerMatches, closureScopeAllows } from './closureAuth.js';
...
export function sessionAuth(hubToken: string | undefined): RequestHandler | null {
  ...
  return (req, res, next) => {
    // This marker is server-created, never a body/header assertion. It
    // distinguishes a signed User login session from the legacy hub token.
    res.locals.irisSession = sessionMatches(bearer(req.header('authorization')))
      || sessionMatches(parseCookies(req.headers.cookie)[COOKIE_NAME]);
    ...
    // Closure-client bearer, checked last and scoped hard: GET only, and only
    // the read-only ledger endpoints the merge/deploy closure scripts call.
    // It never reaches /api/session above (that block returns earlier), so it
    // cannot log in or mint a session, and res.locals.irisSession stays false.
    if (closureScopeAllows(req.method, req.path)
      && closureBearerMatches(hubToken, bearer(req.header('authorization')))) {
      return next();
    }
