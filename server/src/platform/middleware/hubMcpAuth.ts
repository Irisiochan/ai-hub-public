 ...
 *
 * Module-turn invocation bearer 与 per-contact bearer 并存（见
 * agents/moduleAuthority.ts 的 sign/verify）：room 模块轮次注入的是签名的
 * {contact, room, module, revision} 作用域 token，hub 路由按请求（含 SSE
 * followup）重验并约束委派范围；DM 轮次沿用下面的 per-contact token。
 */
