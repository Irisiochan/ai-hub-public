...
import type { GrokRuntimeHome } from './grokRuntimeHome.js';
...
export class GrokCliBackend implements AgentBackend {
  ...
  private spawnAttempt(): void {
    ...
    if (this.opts.effort && ['low', 'medium', 'high', 'xhigh', 'max', 'ultra'].includes(this.opts.effort)) {
    ...
    if (this.opts.runtimeHome) {
      env.GROK_HOME = this.opts.runtimeHome.home;
      env.GROK_AUTH_PATH = this.opts.runtimeHome.authPath;
      env.GROK_FOLDER_TRUST = '1';
      // The generated home is the authority for this turn, not ambient
      // Claude/Cursor MCP definitions or a shared leader's cached config.
      env.GROK_CLAUDE_MCPS_ENABLED = 'false';
      env.GROK_CURSOR_MCPS_ENABLED = 'false';
      env.GROK_CODEX_MCPS_ENABLED = 'false';
    }
