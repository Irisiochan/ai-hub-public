  ...
  async *stream(
    ...
    const res = await fetch(this.config.baseUrl.trim(), {
      ...
      headers: {
        ...
        ...openCodeGoHeaders(this.config.baseUrl, this.sessionId),
      },
      body: JSON.stringify({
...
/** Go accepts third-party agents with their own identity and stable sessions. */
export function openCodeGoHeaders(baseUrl: string, sessionId: string): Record<string, string> {
  const url = new URL(baseUrl.trim());
  if (url.protocol !== 'https:' || url.hostname !== 'opencode.ai'
    || (url.port && url.port !== '443') || !url.pathname.startsWith('/zen/go/')) return {};
  return { 'user-agent': 'ai-hub/0.3.3', 'x-opencode-session': sessionId };
}
