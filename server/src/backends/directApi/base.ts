...
import type { Db, MessageRow, GatewayTool } from '../../platform/index.js';
import {
  gemHeartbeatHistory,
  compactSummaryText,
...
import { type VaultClient, timestampedMessage } from '../../memory/index.js';
import { quotedRoomMessage } from '../../rooms/index.js';
import {
  AsyncQueue,
  type AgentBackend,
...
export { estimateTokens } from '../../prompt/index.js';
export {
  compressToolResultForInjection,
  TOOL_RESULT_MAX_CHARS,
} from '../../prompt/index.js';
...
export class DirectApiBackend implements AgentBackend {
  ...
  private createProvider(requestModel: string): DirectApiProvider<any> {
    const config = {
      ...
      // API history is DB-backed. Keep routing stable across turns/backend
      // recreation, while isolating DMs and each responding room member.
      sessionId: 'ai-hub-' + createHash('sha256').update(JSON.stringify([
        this.opts.contactId, this.opts.roomMode?.selfId ?? this.opts.memberId ?? '',
      ])).digest('hex'),
    };
