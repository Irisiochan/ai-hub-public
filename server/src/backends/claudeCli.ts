...
export function claudeTurnText(text: string, imagePaths: string[] = []): string {
  ...
  return [
    ...
    '</本轮图片附件>',
    '请先使用 Read 工具逐张读取以上图片，再结合图片内容回答。若读取失败，请明确报告具体错误；不要只根据文字上下文猜图。',
  ].join('\n');
}

export function claudePermissionDecision(
  request: any,
  allowedImagePaths: Iterable<string>
): { behavior: 'allow'; updatedInput: Record<string, unknown> } | { behavior: 'deny'; message: string } {
  const input = request?.input;
  const requestedPath = typeof input?.file_path === 'string' ? path.resolve(input.file_path) : '';
  const allowed = new Set([...allowedImagePaths].map((candidate) => path.resolve(candidate)));
  if (request?.tool_name === 'Read' && requestedPath && allowed.has(requestedPath)) {
    return { behavior: 'allow', updatedInput: input };
  }
  return {
    behavior: 'deny',
    message: `ai-hub 聊天模式没开放 ${request?.tool_name ?? '该工具'}，或请求路径不属于本轮附件。`,
  };
}
...
export class ClaudeCliBackend implements AgentBackend {
  ...
  private allowedImagePaths = new Set<string>();
  ...
  private spawn(resumeToken: string | null): Promise<boolean> {
    ...
    if (this.opts.sshAllowed === false) {
      // Module turns without ssh: remove ambient SSH agent credentials so
      // agent/ssh-based remote operations genuinely fail. Residual risk:
      // on-disk keys are outside process-env control (see moduleAuthority).
  ...
  sendTurn(input: TurnInput): TurnHandle {
    ...
    this.allowedImagePaths = new Set((input.imagePaths ?? []).map((candidate) => path.resolve(candidate)));
    ...
    const sent = this.proc?.send({
      type: 'user',
      message: { role: 'user', content: claudeTurnText(input.text, input.imagePaths) },
    });
    if (!sent) {
  ...
  /** Permission asks: only the exact persisted image paths for this turn may use Read. */
  private handleControlRequest(line: any): void {
    const req = line?.request;
    if (req?.subtype !== 'can_use_tool') return;
    const decision = claudePermissionDecision(req, this.allowedImagePaths);
    this.opts.log(`${decision.behavior === 'allow' ? 'allowing' : 'denying'} tool request: ${req.tool_name}`);
    this.proc?.send({
