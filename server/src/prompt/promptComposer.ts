...
import type { MemoryConfig, ContactRow } from '../platform/index.js';
import {
...
import { delegationGuidance, type DelegationCfg } from '../jobs/index.js';
import { MessageRepo, historicalMessageText } from '../messages/index.js';
import { ROOM_RHYTHM_TEMPLATE } from '../rooms/index.js';
import { estimateTokens } from './tokenEstimate.js';
import type { LifeEventService } from '../companion/index.js';
...
export class PromptComposer {
  constructor(
    ...
    /** Workflow-only master: suppresses cross-contact state blocks. Restart-required. */
    private readonly workflowOnlyEnabled: boolean = false
  ) {}
  ...
  async composeTurn(
    ...
    // 群聊有独立转录语义，只进 DM。
    const lifeBlock = !ctx.isRoom && !this.workflowOnlyEnabled
      ? this.lifeEvents?.turnBlock(ctx.agent) ?? ''
      : '';
    if (lifeBlock) {
      turnText = [turnText, '', lifeBlock].join('\n');
      ctx.log(`cross-contact state injected (${lifeBlock.length} chars)`);
    }
    // 刀1：当天 diary per-turn 注入，紧邻 P3 S3 旁路。群聊 v1 不注入；
    // composeStart / 静态 preamble 不得含 diary 正文（cache 前缀稳定）。
    if (!ctx.isRoom && this.vault) {
      try {
        const diaryBlock = await getSameDayDiaryBlock(this.vault);
        if (diaryBlock) {
          turnText = [turnText, '', diaryBlock].join('\n');
          ctx.log(`same-day diary injected (${diaryBlock.length} chars)`);
        }
      } catch {
        // 文件缺失或 Vault 故障：静默省略。
      }
