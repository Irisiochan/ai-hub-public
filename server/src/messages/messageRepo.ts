import type { Db, MessageOrigin, MessageRow } from '../platform/index.js';
...
export class MessageRepo {
  ...
  constructor(db: Db) {
    this.statements = {
      ...
      insertKeyed: db.prepare(
        `INSERT INTO messages (contact_id, sender, role, kind, content, status, turn_id, meta, origin, idempotency_key)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ),
      idByKey: db.prepare(
        'SELECT id FROM messages WHERE contact_id = ? AND idempotency_key = ? ORDER BY id DESC LIMIT 1'
      ),
      toolUseContents: db.prepare(
  ...
  /** 幂等键已存在时返回既有行 id；用于「每轮最多一条」的事实行。 */
  idByIdempotencyKey(contactId: string, key: string): number | undefined {
    const row = this.statements.idByKey.get(contactId, key) as { id: number } | undefined;
    return row ? Number(row.id) : undefined;
  }

  /** 带幂等键的写入：同 key 重复写会被 UNIQUE 约束挡下，调用方先查 idByIdempotencyKey。 */
  insertKeyed(
    contactId: string,
    sender: string,
    ...
    const result = this.statements.insertKeyed.run(
      contactId,
      sender,
      fields.role,
      fields.kind,
  ...
  /** 某一轮里所有工具调用的正文，供心跳回执拼装。 */
  toolUseContents(contactId: string, turnId: string): string[] {
    return (this.statements.toolUseContents.all(contactId, turnId) as Array<{ content: string }>)
      .map((row) => row.content);
  }
