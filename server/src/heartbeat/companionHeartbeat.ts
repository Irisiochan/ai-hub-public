...
import { heartbeatReceipt, heartbeatWriteTool, retryableHeartbeatError } from '../companion/index.js';
import { hardDeleteMessages, automationMeta } from '../messages/index.js';
import {
  type HubConfig,
  type ContactRow,
  type Db,
  type HeartbeatSessionRow,
  type MessageRow,
  type HubLogger,
  type SseHub,
...
export class CompanionHeartbeat {
  ...
  private workflowOnly(): boolean {
    return isWorkflowOnlyEnabled(this.deps.config);
  }
  ...
  start(): void {
    ...
    // Workflow-only master: preserve session audit history but never tick.
    // No restart or manual HTTP call may reactivate while enabled.
    if (this.workflowOnly()) {
      this.log('heartbeat start suppressed: workflow-only mode', {});
      return;
    }
    void this.runOnce();
  ...
  startSession(contactId: string, minutes: number | null): HeartbeatStatus {
    // Workflow-only master: manual HTTP reactivation stays blocked; audit kept.
    if (this.workflowOnly()) {
      throw new HeartbeatError('workflow-only 模式已开启，心跳已暂停；需先关闭该模式并重启网关', 400);
    }
  ...
  async runOnce(): Promise<void> {
    // Workflow-only: old sessions cannot resume via restart scan. History kept.
    if (this.workflowOnly()) return;
  ...
  private async tickIfDue(row: HeartbeatSessionRow): Promise<void> {
    // Defense in depth: direct tick calls also stay quiet in workflowOnly.
    if (this.workflowOnly()) return;
    const now = Date.now();
