...
import { attachmentPathsForMessages } from '../attachments.js';
import {
  deactivateSession,
  getActiveSession,
  getLastSeen,
...
export class AgentRuntime {
  ...
  private async ensureStarted(): Promise<void> {
    ...
    if (this.agent.backend === 'claude-cli') {
      ...
      let mcpConfig: string | undefined = cfg.mcpConfig ?? undefined;
      if (delegationOn) {
        // hub MCP 端点合并进联系人的 mcp 配置，模型才真的有 delegate 工具可调
        mcpConfig = this.writeHubMcpConfig(cfg.mcpConfig, cfg.cwd);
      ...
      this.backend = new DirectApiBackend({
        ...
        historyTokenBudget: Math.max(Number(cfg.historyTokenBudget ?? 24000), 2048),
        minRecentTurns: Math.max(Number(cfg.minRecentTurns ?? 6), 1),
        summaryMaxTokens: Math.max(Number(cfg.summaryMaxTokens ?? 3000), 256),
        historySummaryStrategy: cfg.historySummaryStrategy === 'off' ? 'off' : 'extractive',
  ...
  /** 把联系人自己的 mcp 配置（路径或内联 JSON）和网关的 hub 委派端点合并成一个配置文件。 */
  private writeHubMcpConfig(base: string | undefined, cwdName: string | undefined): string {
    let servers: Record<string, unknown> = {};
    if (base) {
      try {
        const raw = base.trim().startsWith('{') ? base : fs.readFileSync(base, 'utf-8');
        servers = { ...(JSON.parse(raw).mcpServers ?? {}) };
      } catch (e: any) {
        this.log(`base mcp config unreadable (${e.message}) — hub-only mcp config`);
      }
    }
    const { config } = this.deps;
    servers.hub = {
      type: 'http',
      url: `http://${config.host}:${config.port}/api/hub-mcp/${this.agent.id}`,
    };
    const dir = path.resolve(config.agentsDir, cwdName ?? this.agent.id);
    fs.mkdirSync(dir, { recursive: true });
    const file = path.join(dir, 'mcp.hub.json');
    fs.writeFileSync(file, JSON.stringify({ mcpServers: servers }, null, 2), 'utf-8');
    return file;
  }
  ...
  async invalidateCliContext(): Promise<void> {
    if (this.agent.backend === 'api') {
      ...
      this.log('API rolling summary invalidated (edit/delete)');
      return;
    }
    deactivateSession(this.deps.db, this.convo.id, this.isRoom ? this.memberId : undefined);
    if (this.isRoom) {
