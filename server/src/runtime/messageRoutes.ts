...
import { isWorkflowRoomConfig } from '../workflow/index.js';
import {
...
export function messagesRouter(
  ...
  r.post('/:id/room-host/messages', (req, res) => {
    ...
    // Retired: the old host write entry is closed for workflow rooms, with
    // one narrow exception: a trigger=false note without coordination meta
    // persists a wake-free record (ancillary digests) but never wakes a
    // model. Anything that would wake (trigger omitted/true) or carry
    // coordination authority is refused outright — omitting `coordination`
    // must not bypass retirement into a sender=room-host wake. Social rooms
    // keep the generic path below. History rows stay readable via GET.
    const coordinationSupplied = req.body?.coordination !== undefined;
    const wakeRequested = req.body?.trigger !== false;
    try {
      const cfg = JSON.parse(contact.config || '{}') as Record<string, unknown>;
      if (isWorkflowRoomConfig(cfg)) {
        if (coordinationSupplied || wakeRequested) {
          return res.status(410).json({
            error: 'room-host messages are retired for workflow rooms; use the task ledger (task_create/task_handoff/execution_start) instead',
          });
        }
      } else if (coordinationSupplied) {
        return res.status(410).json({
          error: 'room-host coordination dispatch is retired; use the task ledger (task_create/task_handoff/execution_start) instead',
        });
      }
    } catch {
      return res.status(410).json({ error: 'room-host messages are retired for workflow rooms' });
    }
    ...
    const workflowModule = coordination?.kind === 'execution' && jobs
      ? (() => {
          const invocation = jobs.workflowModules.invoke(
            'execute',
            coordination.taskPath,
            coordination.planHash,
          );
          return {
            moduleId: 'execute',
