...
import type { ContactRow, Db, MessageRow, HubLogger, SseHub } from '../platform/index.js';
import { shanghaiStamp } from '../memory/index.js';
import { contactConfig } from '../contacts/index.js';
import { resolveWorkflowOrchestratorId } from './moduleAuthority.js';
import { isWorkflowRoomConfig, WorkflowModulesStore } from './workflowModules.js';
...
export function dispatchCoordinationRoomHost(
  ...
  // social rooms keep the legacy room-config resolution.
  const targetId = input.targetId ?? resolveWorkflowOrchestratorId(new WorkflowModulesStore(deps.db), room);
 ...
 * Workflow rooms are skipped: the plan module binding governs them and the
 * legacy per-room orchestrator key is ignored there by design.
...
export function auditRoomOrchestratorConfigs(db: Db): RoomOrchestratorConfigIssue[] {
  ...
  for (const room of rooms) {
    ...
    try {
      if (isWorkflowRoomConfig(cfg as unknown as Record<string, unknown>)) continue;
    } catch {
      // Fall through to the legacy audit when the config is unreadable.
    }
    ...
    const resolved = resolveWorkflowOrchestratorId(new WorkflowModulesStore(db), room);
