...
import { buildDeliveryChecks } from './deliveryChecks.js';
...
const DELIVERY_CONTRACT = [
  ...
  'For implement/fix tasks, keep the change minimal and the tests proportionate to the approved scope. Do not treat process exit, a commit, or a self-reported completion as independent review acceptance.',
  ...
  'If SSH permission is absent but remote deployment is required, file one deploy-tail with the exact host, checkout, service, and verification steps — but only for a candidate that already passed independent review. An implement/fix attempt still awaiting review must not file a deploy-tail; list the deploy steps in the receipt and the owner registers one tail for the approved final candidate.',
...
export function workersRouter(
  ...
  r.get('/workflow-profiles', (req, res) => {
    // Legacy read path stays compatible: a single synthetic fixed profile
    // derived from current module bindings, for existing worker stage display.
    // Profile switching is disabled (410); use /api/workflow-modules bindings.
    const bindings = jobs.workflowModules.bindings();
    const primary = (module: 'plan' | 'execute' | 'review' | 'maintenance') => {
      const binding = bindings[module];
      return { runner: binding.runner, model: binding.model, reasoning: binding.reasoning };
    };
    const synthetic = {
      id: 'fixed-modules',
      version: 1,
      label: 'Fixed workflow modules',
      description: 'Sole fixed workflow: plan/execute/review/arbitration/merge/deploy/maintenance module bindings.',
      routes: {
        plan: { primary: primary('plan') },
        review: { primary: primary('review'), escalateAfter: 3 },
        execute: { primary: primary('execute'), escalateAfter: 5 },
        fix: { primary: primary('execute'), escalateAfter: 5 },
        maintenance: { primary: primary('maintenance') },
        patrol: { primary: primary('maintenance') },
      },
  ...
  r.post('/workflow-profiles/preview', (_req, res) => {
    return res.status(410).json({ error: 'workflow profiles are fixed; use PATCH /api/workflow-modules/:moduleId bindings' });
  });

  r.post('/workflow-profiles/switch', (_req, res) => {
    return res.status(410).json({ error: 'workflow profile switching is disabled; use PATCH /api/workflow-modules/:moduleId bindings' });
  });
  ...
  r.post('/jobs', (req, res) => {
    ...
    const workflow = jobs.workflowModules.invoke(moduleForStage(stage), taskPath,
      suppliedFingerprint ?? problemFingerprint(prompt, taskPath));
    const explicitRunner = ['claude', 'codex', 'grok', 'opencode'].includes(req.body?.runner) ? req.body.runner : null;
  ...
  r.post('/jobs/:id/quality', (req, res) => {
    ...
    if (jobs.workflowModules.isFenced(job.id)) {
      return res.status(409).json({ error: 'job fenced by takeover; late quality rejected' });
    }
    ...
    // Primary record: fixed modules (2 inadequates → arbitration, 3rd → human;
    // review 3rd → human; infrastructure never counts). Legacy profile record
    // is kept so old snapshots stay readable; the module store dedupes
    // migrated job ids so nothing counts twice.
    let moduleInvocation: { moduleId: 'plan' | 'execute' | 'review' | 'arbitration' | 'merge' | 'deploy' | 'maintenance'; taskPath: string; problemFingerprint: string };
    try {
      const options = JSON.parse(job.options || '{}') as Record<string, unknown>;
      const stored = options.workflowModule as { moduleId?: string } | undefined;
      const known = ['plan', 'execute', 'review', 'arbitration', 'merge', 'deploy', 'maintenance'];
      const moduleId = stored && known.includes(String(stored.moduleId)) ? String(stored.moduleId) : 'execute';
      moduleInvocation = {
        moduleId: moduleId as never,
        taskPath: typeof options.taskPath === 'string' ? options.taskPath : '',
        problemFingerprint: typeof options.problemFingerprint === 'string' ? options.problemFingerprint : '',
      };
    } catch {
      moduleInvocation = { moduleId: 'execute', taskPath: '', problemFingerprint: '' };
    }
    const moduleOutcome = quality === 'success' && moduleInvocation.moduleId === 'review'
      ? { counted: false, reason: 'independent frozen-SHA acceptance is recorded by the closure gate' }
      : jobs.workflowModules.record(
      { id: job.id },
      moduleInvocation,
      { quality, detail },
    );
    if ('reason' in moduleOutcome && moduleOutcome.reason === 'job quality already recorded') {
      return res.status(409).json({ error: moduleOutcome.reason });
    }
    jobs.addMessage(job.id, 'User', 'quality', JSON.stringify({ quality, module: moduleOutcome }));
    const escalated = (moduleOutcome as { escalateToHuman?: boolean }).escalateToHuman === true;
    if (escalated) {
      const streak = (moduleOutcome as { streak?: number }).streak ?? 0;
      jobs.updateDelivery(job.id, 'workflow', {
        stage: 'user_decision',
        summary: moduleInvocation.moduleId === 'review'
          ? 'review 连续 3 次效果不佳，已停止自动派单，等 User 决定。'
          : `连续 ${streak} 次实现效果不佳，已停止自动派单，等 User 决定。`,
        nextOwner: 'User',
        blocker: 'workflow_streak_escalation',
      });
    }
    res.json({ ok: true, ...moduleOutcome });
  });
  ...
  // Read-only self status for the Windows launcher: /api/workers needs a hub
  // session, but the launcher only holds this worker's token.
  r.get('/worker/me', (req, res) => {
    const worker = workerFrom(req, db);
    if (!worker) return res.status(401).json({ error: 'invalid worker token' });
    res.json({ worker: publicWorker(worker) });
  });
  ...
  const tryClaim = (worker: WorkerRow): JobRow | null => {
    ...
    for (const job of candidates) {
      if (jobs.workflowModules.isFenced(job.id) || jobs.blockUnavailablePool(job)) continue;
  ...
  r.get('/worker/reconcile', (req, res) => {
    ...
    res.json({ jobs: rows.filter((job) => !jobs.workflowModules.isFenced(job.id)).map(publicJob) });
  });
  ...
  r.post('/worker/jobs/:id/events', (req, res) => {
    ...
    if (jobs.workflowModules.isFenced(job.id)) {
      return res.status(409).json({ error: 'job fenced by takeover; late events rejected' });
    }
  ...
  r.post('/worker/jobs/:id/complete', (req, res) => {
    ...
    const deliveryMeta = deliveryWithChecks ? boundedDeliveryMeta(deliveryWithChecks) : null;
    ...
    let qualityOutcome: ReturnType<typeof jobs.workflowModules.record> | undefined;
    if (outcome.changed && quality && ['success', 'inadequate', 'infrastructure'].includes(quality)) {
      const finished = jobs.get(job.id)!;
      let moduleInvocation: { moduleId: 'plan' | 'execute' | 'review' | 'arbitration' | 'merge' | 'deploy' | 'maintenance'; taskPath: string; problemFingerprint: string } = { moduleId: 'execute', taskPath: '', problemFingerprint: '' };
      try {
        const options = JSON.parse(finished.options || '{}') as Record<string, unknown>;
        const stored = options.workflowModule as { moduleId?: string } | undefined;
        const known = ['plan', 'execute', 'review', 'arbitration', 'merge', 'deploy', 'maintenance'];
        if (stored && known.includes(String(stored.moduleId))) {
          moduleInvocation = {
            moduleId: String(stored.moduleId) as never,
            taskPath: typeof options.taskPath === 'string' ? options.taskPath : '',
            problemFingerprint: typeof options.problemFingerprint === 'string' ? options.problemFingerprint : '',
          };
        }
      } catch { /* keep default */ }
      qualityOutcome = quality === 'success' && moduleInvocation.moduleId === 'review'
        ? { counted: false, reason: 'independent frozen-SHA acceptance is recorded by the closure gate' }
        : jobs.workflowModules.record(
        { id: finished.id },
        moduleInvocation,
