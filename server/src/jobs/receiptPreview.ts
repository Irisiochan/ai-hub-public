...
/**
 * Legacy waiting-review gate shape (read-only, grants no authority): only jobs
 * at the real review gate skip the worker-tail projection (the harness returns early for
 * them, so no tail file exists). Every condition matters — route implement/fix
 * + write=true + blocked + blocked_unpushed + normalized declared stage
 * waiting_review — so a review-route job or a blocked_local_changes job that
 * merely declares waiting_review still keeps its tail fallback.
 */
export function isWaitingReviewGate(job: JobRow): boolean {
  try {
    const options = job.options ? JSON.parse(job.options) : {};
    const permissions = job.permissions ? JSON.parse(job.permissions) : {};
    const routeClass = typeof options.routeClass === 'string' ? options.routeClass.trim().toLowerCase() : '';
    if (!['implement', 'fix'].includes(routeClass)) return false;
    if (permissions.write !== true) return false;
    if (job.status !== 'blocked' || job.delivery_state !== 'blocked_unpushed') return false;
    const meta = deliveryMeta(job);
    const declared = record(meta.declared);
    const stage = typeof declared.stage === 'string' ? declared.stage.trim().toLowerCase().replace(/-/g, '_') : '';
    return stage === 'waiting_review';
  } catch {
    return false;
  }
}
...
export function formatWorkerReceiptPreview(
  ...
  return boundedReceipt([
    ...
    compact(options.closing ?? 'preview 只是索引，不能代替 diff/测试证据与正式结论；需要逐项证据时先 recall 完整回执。', 180),
  ], recall);
}
