...
const upstream = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    const text = rec.model === 'slow-model' ? '慢成员不该被单点名触发' : '示例助手正常';
...
try {
  addContact('gala', '示例助手', 'gala-model');
  ...
  requests.length = 0;
  const roomInsert = db.prepare(`INSERT INTO messages (contact_id, sender, role, kind, content, status) VALUES ('room', 'user', 'user', 'text', '@示例助手 群聊测速', 'done')`).run();
  const roomStart = Date.now();
  const targets = manager.dispatchRoomMessage(contact('room'), '@示例助手 群聊测速');
  ...
  assert(roomStatuses.some((s) => s.member === '示例助手' && s.state === 'thinking'));
  assert(!roomStatuses.some((s) => s.member === '慢成员'), 'slow member should not enter room status for a single @示例助手 turn');
