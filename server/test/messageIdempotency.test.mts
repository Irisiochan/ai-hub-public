...
try {
  const latestMigration = loadMigrationFiles(path.resolve(here, '../migrations')).at(-1)!;
  assert.equal(upgraded.pragma('user_version', { simple: true }), latestMigration.version);
